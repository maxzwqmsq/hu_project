$(function () {
    ...
    function selectHouse() {
        ...
        $.ajax({
            url: url1 + "?opt=selectHouses&city="+city+"&zhoubian=2&type=0&row=10&" + requestData,
            type: 'post',
            ...
            if (res.code === 200) {
                ...
                if (res.data.iszhoubian !== 0) {
                    ...
                    $(".c-area li").click(function () {
                        if ($(this).text() === "南京周边") {
                            $(".area-site").empty();
                            $(".area-site").append("<li class='active'><a href='#'>不限</a></li>");
    ...
    $roomlist = $(".roomlist");
    $.ajax({
        url: url1 + "?opt=selectHouses&city="+city+"&zhoubian=2&type=0&row=10&page=" + page,
        type: "post",
        success: function (res, status) {
            initHouseData(res);
        }
    ...
    function initHouseData(res) {
        ...
        for (var i = 0; i < res.data.length; i++) {
            ...
            $(".roomlist").append("<li class='dp_info tag" + page + i + "'><p class='dp_tx'><img src='images/21.png'>"
                + "</p>"
                ...
                + "</span></p></li>");
                if(typeof(res.data[i].dp_list.pic_dp) === "undefined"){
                    $(".dp_tx img").attr("src", "images/21.png");
                }else{
            ...
            if (res.data[i].sc_status === 0) {//未收藏
                $(".shoucang img").attr("src", "images/sc.png");
            } else {
                $(".shoucang img").attr("src", "images/关注后.png");
            }

            //判断有没有点评
            if(res.data[i].dp_list.length===undefined){
                    console.log(i);
                    var str2="dp_info.tag" + page + i;
                    $("."+str2).hide();
            }
    ...
    $(window).scroll(function () {
        ...
        var scrollHeight = $(".roomlist").height()+$(".nav").height()+$(".tab").height();//内容高度
    ...
    /*var wxurl1="https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx6abc12345add9a62&redirect_uri=http://www.njjinmaiyun.com/loupan.html&response_type=code&scope=snsapi_base&state=1#wechat_redirect";
    $(window).attr("location",wxurl1);*/
    ...
    // alert("code:"+code);
    //获取openid
    /*var wxurl2="https://api.weixin.qq.com/sns/oauth2/access_token?appid=wx6abc12345add9a62&secret=9100be1293e57ae49372cf8fce92fd8e&code="+code+"&grant_type=client_credential";
    $.ajax({
        url:wxurl2,
        type:"post",
        success:function (res,status) {
